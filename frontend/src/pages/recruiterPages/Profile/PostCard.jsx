...
function PostCard({ post, forPostCard }) {
  ...
  const [recruiterDeletePost] = useRecruiterDeletePostMutation();
  const [recruiterEditPost] = useRecruiterEditPostMutation();
  ...
  const handleDeletePost = async (postId) => {
    try {
      const response = await recruiterDeletePost({ postId: postId });
  ...
  const handleEditPost = async () => {
    try {
      const response = await recruiterEditPost({
        postId: post._id,
  ...
  return (
    <div class="card-body">
      ...
      <div class="card-footer">
        ...
        <div className="d-inline-block text-muted ml-4">
          <small class="align-middle" style={{ cursor: "pointer" }}>
            <strong onClick={() => setShowEdit(true)}>
